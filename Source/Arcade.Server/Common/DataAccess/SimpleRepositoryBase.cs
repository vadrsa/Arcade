...
namespace Common.DataAccess
{
    ...
    public abstract class SimpleRepositoryBase<T, K, R, C> : RepositoryBase<T, R, C>, IRepository<T, K, R>
        where T : IDEntityBase<K>
        where R : class
        where C : DataConnection
    {
        public SimpleRepositoryBase(C context) : base(context)
        {
        }
        ...
        public virtual T Insert(T obj)
        {
            ...
            {
                if(obj.Id == null)
                    obj.Id = (K)(object)Guid.NewGuid().ToString();
                Context.Insert(obj);
            }
            else
            {
                object indentity = Context.InsertWithIdentity(obj);
